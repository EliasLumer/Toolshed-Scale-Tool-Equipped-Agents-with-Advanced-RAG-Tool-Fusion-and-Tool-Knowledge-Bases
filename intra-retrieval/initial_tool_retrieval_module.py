...
class InitialToolRetrievalModule(BaseARTFModules):
    def __init__(self, embedder: Optional[object], toolshed_knowledge_base: BaseVectorStoreIndexer):
    ...
    def generate(self, query: str, top_k: int) -> List[Document]:
        """Queries the FAISS index using the provided query and returns the top_k results."""
        try:
    ...
    async def agenerate(self, query: str, top_k: int) -> List[Document]:
        """Asynchronous query to the FAISS index."""
        try:
